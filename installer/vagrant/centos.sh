...
# RaBe Airtime Packages (some builddeps are bundle closely with that legacy upstream build)
...
# and now for some rpm dev tooling :)
yum install -y rpmdevtools yum-utils

# install builddeps as listend in specfile
yum-builddep /vagrant/libretime.spec

# rpmbuild dirs in /home/vagrant
su -l -c rpmdev-setuptree vagrant
